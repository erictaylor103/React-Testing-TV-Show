...
test('App renders wothout errors', async () => {
  
  mockFetchShow.mockResolvedValueOnce(strangerData);

  const { getByText } = render(<App />);

  // App renders with no error / confirmed by checking if the select a season text is rendered on the screen (since this is the last element to render then we confirm that the page loaded completely)
  await waitFor(() => {
    expect(getByText(/select a season/i));
  });
});

test("A season and a season episode can be selected ", async () => {
  mockFetchShow.mockResolvedValueOnce(strangerData);
  ...
  // mock when a user clicks the select a season button
  ...
  // when we choose season 1 the total of 8 episodes renders with no errors / we check by looking for the "Episode" text in the Season 1 list
